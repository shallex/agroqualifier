...
class MandarinSegmentationModel(L.LightningModule):
    ...
    def log_images(self, images, preds, targets):
        images_to_show = []
        caption = []
        for i in range(min(5, len(images))):
            ...
            # Predicted image
            pred_boxes = preds[i]["boxes"].long()
            pred_labels = [f"Score: {score:.3f}" for score in preds[i]["scores"]]
            output_image = draw_bounding_boxes(image, pred_boxes, pred_labels, colors="red")
            output_image = self.draw_mask(output_image, preds[i])
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx == 0:
            self.log_images(images, preds, targets)
